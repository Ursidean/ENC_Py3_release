...
points_file = output_path + case_study + "\\Rules\\INC_points.txt"
points = np.loadtxt(points_file)
# Load the tails file.
tails_file = output_path + case_study + "\\Rules\\INC_tails.txt"
...
if seed is True:
    ...
    point_levels_file_path = (base_path + "EU_output\\" +
                              case_study +
                              "\\Rules\\HVD_point_levels.txt")
    # Specify the tail levels file path
    tail_levels_file_path = (base_path + "EU_output\\" +
                             case_study +
    ...
    theta_cp = 0.050
    theta_ct = 0.008
    rules = seed_rules2(luc_names, luc, act, pas,
                        point_levels, tail_levels,
                        theta_st, theta_cp, theta_ct)
else:
...
while run_count < max_runs:
    ...
    # Calculate the AAWCE.
    clu_temp[run_count] = w_clu_error(amap, smap, mask,
                                      clu_weights, luc,
                                      pas, act)
    # Calculate the run Fuzzy Kappa.
    kappa_temp[run_count] = kappa(amap, smap, mask)
    # Calculate the run Fuzzy Kappa Sim.
    kappa_sim_temp[run_count] = ksim(omap, amap, smap, mask)
    ...
    if iterations_counter > max_iterations:
        ...
        while (abs(a - b) > gss_ip_tol):
            ...
            while run_count < max_runs:
                ...
                # Calculate the run WCE.
                clu_temp_c[run_count] = w_clu_error(amap, smap, mask,
                                                    clu_weights, luc,
                                                    pas, act)
                # Calculate the run Fuzzy Kappa.
                kappa_temp_c[run_count] = kappa(amap, smap, mask)
                # Calculate the run Fuzzy Kappa Simulation.
                kappa_sim_temp_c[run_count] = ksim(omap, amap, smap, mask)
            ...
            ws_c = ws_3_metrics(kappa_avg_c, kappa_sim_avg_c,
                                clu_avg_c,
            ...
            while run_count < max_runs:
                ...
                # Determine objectives.
                # Calculate the run WCE.
                clu_temp_d[run_count] = w_clu_error(amap, smap, mask,
                                                    clu_weights, luc,
                                                    pas, act)
                # Calculate the run Fuzzy Kappa.
                kappa_temp_d[run_count] = kappa(amap, smap, mask)
                # Calculate the run Fuzzy Kappa Simulation.
                kappa_sim_temp_d[run_count] = ksim(omap, amap, smap, mask)
                # Add one to iterator to prevent infinite loop!
                run_count = run_count + 1
            # Find the average over the number of runs performed.
...
print("Fine tuning the cross-influence tails")
# Evaluate the socio-economic groups to determine the order for
# interactive rules. First, allocate a numerical value depending on
# class converted to.
# Rank and determine order. Start by initialising an array to track order.
temp_ct_order = np.zeros(shape=(luc, act))
for i in range(0, luc):
    for j in range(0, act):
        if i == j + pas:
            # Skip if an inertia point.
            ...
            if socio_eco_group[j + pas] == "Residential":
                ...
                temp_ct_order[i, j] = 1
# Rank and determine order. Start by initialising an array to track order.
ct_order = np.zeros(shape=(luc, act))
# Iterate through and assign rank. Start at 1.
ranking = 1
iterator = 1
while iterator < 5:
    ...
    for i in range(0, luc):
        for j in range(0, act):
            if temp_ct_order[i, j] == iterator:
                # If temp_ct_order equivalent value is found, assign current
                # ranking value.
                ct_order[i, j] = ranking
                # Add one to the ranking value.
                ranking = ranking + 1
    # Add one to the iterator.
    iterator = iterator + 1
# Apply the Golden Section Search to interactive tails.
while np.sum(ct_order) > 0:
    if iterations_counter > max_iterations:
        # Set values to 0 to prevent further testing.
        # Identify max value, corresponding to parameter that will be
        # tested.
        par_index = unravel_index(ct_order.argmax(),
                                  ct_order.shape)
        # Set the corresponding value in the list to 0.
        ct_order[par_index[0], par_index[1]] = 0
    else:
        # Identify max value, corresponding to parameter that will be
        # tested.
        par_index = unravel_index(ct_order.argmax(),
        ...
        rule_name = ("from " + luc_names[lu_elem] +
                     " to " + luc_names[fu_elem + pas])
        print("Calibrating rule: " + rule_name)
        if tails[lu_elem, fu_elem] == 1:
            print("Attraction rule")
        elif tails[lu_elem, fu_elem] == -1:
            print("Repulsion rule")
        # Track the rule being adjusted
        rule_from_tracker.append(lu_elem)
        rule_to_tracker.append(fu_elem)
        pt_tracker.append(1)
        ...
        xe = adjust_rule_values[3]
        # Apply the Golden Section Search to optimise the output.
        # Set bounds based on attraction or repulsion.
        if tails[lu_elem, fu_elem] == 1:
            a = min_ct
            b = max_ct
        elif tails[lu_elem, fu_elem] == -1:
        ...
        while (abs(a - b) > gss_ct_tol):
            ...
            while run_count < max_runs:
                ...
                clu_temp_c[run_count] = w_clu_error(amap, smap, mask,
                                                    clu_weights, luc,
                                                    pas, act)
                # Calculate the run Kappa.
                kappa_temp_c[run_count] = kappa(amap, smap, mask)
                # Calculate the run Kappa Simulation.
                kappa_sim_temp_c[run_count] = ksim(omap, amap, smap, mask)
                # Add one to iterator to prevent infinite loop!
                run_count = run_count + 1
            # Find the average over the number of runs performed.
            ...
            kappa_sim_avg_d = sum(kappa_sim_temp_d) / len(kappa_sim_temp_d)
            # Now determine the weighted sum.
            ws_d = ws_3_metrics(kappa_avg_d, kappa_sim_avg_d,
                                clu_avg_d,
                                w1, w2, w3, r1, r2, r3, s1, s2, s3)
            # Add step here to break out if objectives are not changing.
            if ws_avg == ws_c and ws_avg == ws_d:
...
output_rules_file = (output_path + "\\" + case_study + "\\Rules\\" +
                     case_study + "_final_rules.csv")
store = [0]*6
